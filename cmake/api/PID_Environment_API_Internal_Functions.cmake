...
macro(build_Environment_Project)
  ...
  add_custom_target(update
    ...
    COMMENT "[PID] INFO : Updating the environment ${PROJECT_NAME} ..."
    VERBATIM
  )
  #########################################################################################################################
...
function(print_Language_Toolset environment lang toolset)
  ...
  if(${environment}_${lang}_TOOLSET_${toolset}_COMPILER)#for compiled languages
    if(${environment}_${lang}_TOOLSET_${toolset}_COMPILER_ID)
      set(lang_str "${lang_str}[PID] INFO :     * compiler : ${${environment}_${lang}_TOOLSET_${toolset}_COMPILER} (id=${${environment}_${lang}_TOOLSET_${toolset}_COMPILER_ID})\n")
    else()
      set(lang_str "${lang_str}[PID] INFO :     * compiler : ${${environment}_${lang}_TOOLSET_${toolset}_COMPILER}\n")
    endif()
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_COMPILER_FLAGS)
    ...
    set(lang_str "${lang_str}[PID] INFO :     * compilation flags : ${flags_str}\n")
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_INCLUDE_DIRS)
    ...
    set(lang_str "${lang_str}[PID] INFO :     * standard library include dirs : ${incs_str}\n")
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_LIBRARY)
    set(lang_str "${lang_str}[PID] INFO :     * standard library : ${${environment}_${lang}_TOOLSET_${toolset}_LIBRARY}\n")
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_INTERPRETER)
    set(lang_str "${lang_str}[PID] INFO :     * interpreter : ${${environment}_${lang}_TOOLSET_${toolset}_INTERPRETER}\n")
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_COMPILER_AR)
    set(lang_str "${lang_str}[PID] INFO :     * archiver : ${${environment}_${lang}_TOOLSET_${toolset}_COMPILER_AR}\n")
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_COMPILER_RANLIB)
    set(lang_str "${lang_str}[PID] INFO :     * static libraries creator : ${${environment}_${lang}_TOOLSET_${toolset}_COMPILER_RANLIB}\n")
  endif()
  if(${environment}_${lang}_TOOLSET_${toolset}_HOST_COMPILER)#for languages that require C/C++ compiler to generate code
    set(lang_str "${lang_str}[PID] INFO :     * host compiler : ${${environment}_${lang}_TOOLSET_${toolset}_HOST_COMPILER}\n")
  endif()
  message("${lang_str}")
...
function(print_Extra_Tool environment tool)
  ...
  if(${environment}_EXTRA_${tool}_PROGRAM)
    set(tool_str "${tool_str}[PID] INFO :     * program: ${${environment}_EXTRA_${tool}_PROGRAM}\n")
  endif()
  if(${environment}_EXTRA_${tool}_PLATFORM_CONFIGURATIONS)
    set(tool_str "${tool_str}[PID] INFO :     * platform requirements: ${${environment}_EXTRA_${tool}_PLATFORM_CONFIGURATIONS}\n")
  endif()
  if(${environment}_EXTRA_${tool}_PLUGIN_BEFORE_DEPENDENCIES
    ...
    set(tool_str "${tool_str}[PID] INFO :     * plugin callbacks:\n")
    if(${environment}_EXTRA_${tool}_PLUGIN_BEFORE_DEPENDENCIES)
      set(tool_str "${tool_str}[PID] INFO :          + before dependencies: ${${environment}_EXTRA_${tool}_PLUGIN_BEFORE_DEPENDENCIES}\n")
    endif()
    if(${environment}_EXTRA_${tool}_PLUGIN_BEFORE_COMPONENTS)
      set(tool_str "${tool_str}[PID] INFO :          + before components: ${${environment}_EXTRA_${tool}_PLUGIN_BEFORE_COMPONENTS}\n")
    endif()
    if(${environment}_EXTRA_${tool}_PLUGIN_DURING_COMPONENTS)
      set(tool_str "${tool_str}[PID] INFO :          + during components: ${${environment}_EXTRA_${tool}_PLUGIN_DURING_COMPONENTS}\n")
    endif()
    if(${environment}_EXTRA_${tool}_PLUGIN_AFTER_COMPONENTS)
      set(tool_str "${tool_str}[PID] INFO :          + after components: ${${environment}_EXTRA_${tool}_PLUGIN_AFTER_COMPONENTS}\n")
    endif()
  endif()
  message("${tool_str}")
...
function(print_Evaluated_Environment environment)
  message("[PID] INFO : description of environment ${environment} solution")
  set(crosscomp_str)
  if(${environment}_CROSSCOMPILATION)#when crosscompiling there are a few specific variables that may be set
    set(crosscomp_str "[PID] INFO : - requires crosscompilation\n")
    if(${environment}_TARGET_SYSTEM_NAME)
      set(crosscomp_str "${crosscomp_str}[PID] INFO :  + target OS: ${${environment}_TARGET_SYSTEM_NAME}")
    endif()
    if(${environment}_TARGET_SYSTEM_PROCESSOR)
      set(crosscomp_str "${crosscomp_str}[PID] INFO :  + target processor: ${${environment}_TARGET_SYSTEM_PROCESSOR}")
    endif()
    if(${environment}_TARGET_SYSROOT)
      set(crosscomp_str "${crosscomp_str}[PID] INFO :  + sysroot: ${${environment}_TARGET_SYSROOT}")
    endif()
    if(${environment}_TARGET_STAGING)
      set(crosscomp_str "${crosscomp_str}[PID] INFO :  + staging: ${${environment}_TARGET_STAGING}")
    endif()
  endif()
  if(crosscomp_str)
  ...
  if(${environment}_SYSTEM_WIDE_CONFIGURATION)
    if(${environment}_LINKER)
      message("[PID] INFO : - linker: ${${environment}_LINKER}")
    endif()
    if(${environment}_AR)
      message("[PID] INFO : - static libraries archiver: ${${environment}_AR}")
    endif()
    if(${environment}_RANLIB)
      message("[PID] INFO : - static libraries creator: ${${environment}_RANLIB}")
    endif()
    if(${environment}_NM)
      message("[PID] INFO : - object symbols extractor: ${${environment}_NM}")
    endif()
    if(${environment}_OBJDUMP)
      message("[PID] INFO : - object symbols explorer: ${${environment}_OBJDUMP}")
    endif()
    if(${environment}_OBJCOPY)
      message("[PID] INFO : - object translator: ${${environment}_OBJCOPY}")
    endif()
    if(${environment}_EXE_LINKER_FLAGS)
      message("[PID] INFO : - linker flags for executables: ${${environment}_EXE_LINKER_FLAGS}")
    endif()
    if(${environment}_MODULE_LINKER_FLAGS)
      message("[PID] INFO : - linker flags for modules: ${${environment}_MODULE_LINKER_FLAGS}")
    endif()
    if(${environment}_SHARED_LINKER_FLAGS)
      message("[PID] INFO : - linker flags for shared libraries: ${${environment}_SHARED_LINKER_FLAGS}")
    endif()
    if(${environment}_STATIC_LINKER_FLAGS)
      message("[PID] INFO : - linker flags for static libraries: ${${environment}_STATIC_LINKER_FLAGS}")
    endif()
    if(${environment}_LIBRARY_DIRS)
      message("[PID] INFO : - system libraries directories: ${${environment}_LIBRARY_DIRS}")
    endif()
    if(${environment}_INCLUDE_DIRS)
      message("[PID] INFO : - system include directories: ${${environment}_INCLUDE_DIRS}")
    endif()
    if(${environment}_PROGRAM_DIRS)
      message("[PID] INFO : - system program directories: ${${environment}_PROGRAM_DIRS}")
    endif()
  endif()

  #now getting all informations about supported languages
  if(${environment}_LANGUAGES)
    message("[PID] INFO : - configured languages:")
    foreach(lang IN LISTS ${environment}_LANGUAGES)
      message("[PID] INFO :   + ${lang}:")
      print_Language_Toolset(${environment} ${lang} 0)
      if(${environment}_${lang}_TOOLSETS GREATER 1)
        message("[PID] INFO :     ++ additionnal toolsets:")
  ...
  if(${environment}_EXTRA_TOOLS)
    message("[PID] INFO : - configured extra tools:")
    foreach(tool IN LISTS ${environment}_EXTRA_TOOLS)
      message("[PID] INFO :   + ${tool}:")
