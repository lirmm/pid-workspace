...
function(resolve_dependency_conflict package dependency first_time)
  ...
  set(str "[PID] WARNING : package ${package} (${${package}_VERSION_STRING}), required by ${required_by_info}, requires '${dependency}' with")
  get_Package_Type(${dependency} PACK_TYPE)
  if(PACK_TYPE STREQUAL "EXTERNAL")
    if(${package}_EXTERNAL_DEPENDENCY_${dependency}_VERSION_SYSTEM${VAR_SUFFIX})
      set(str "${str} system")
    elseif(${package}_EXTERNAL_DEPENDENCY_${dependency}_VERSION_EXACT${VAR_SUFFIX})
      set(str "${str} exact")
    endif()
    set(str "${str} version ${${package}_EXTERNAL_DEPENDENCY_${dependency}_VERSION${VAR_SUFFIX}}")
  else()
    if(${package}_DEPENDENCY_${dependency}_VERSION_EXACT${VAR_SUFFIX})
      set(str "${str} exact")
    endif()
    set(str "${str} version ${${package}_DEPENDENCY_${dependency}_VERSION${VAR_SUFFIX}}")
  endif()
  get_Dependency_Resolution_Path_For(requirement_tree REQ_VERS REQ_EXACT REQ_SYS ${dependency} ${mode})
  if(REQ_SYS)
    set(vers "system version ${REQ_VERS}")
  elseif(REQ_EXACT)
    set(vers "exact version ${REQ_VERS}")
  else()
    set(vers "version ${REQ_VERS}")
  endif()
  message("${str}. But ${vers} is already required by: ${requirement_tree}")
...
if(package STREQUAL "${PROJECT_NAME}")
  ...
  foreach(pack IN LISTS RESOLVE_PACKAGE_DEPENDENCIES_REQUIRED_BY)
    ...
    set(new_pack_info "${pack} (${${pack}_VERSION_STRING})")
    if(required_by_info)
      set(required_by_info "${required_by_info} -> ${package}")
    else()
      set(required_by_info "${new_pack_info}")
    endif()
  endforeach()
  finish_Progress(${GLOBAL_PROGRESS_VAR})
  message(FATAL_ERROR "[PID] CRITICAL ERROR: cyclic dependency detected when resolving dependencies of package ${package}: ${required_by_info} requires ${package}.")
endif()
...
function(try_In_Development_Version DEPLOYED_VERSION package version_to_check is_exact run_tests release_only)
  ...
  if(already_installed)
    get_Version_String_Numbers("${version_to_check}" major minor patch)
    if(DEFINED patch)
      ...
      foreach(vers IN LISTS already_installed)
        if(vers MATCHES "^${major}\\.${minor}.*$")
          list(REMOVE_ITEM already_installed ${vers})
          file(REMOVE_RECURSE ${WORKSPACE_DIR}/install/${CURRENT_PLATFORM}/${package}/${vers})
        endif()
      endforeach()
    endif()
  endif()
  ...
  if(ALL_IS_OK)
    ...
    # installed_after_build should contain the newly built version ONLY
...
if(INDEX EQUAL -1) # selected version not found in versions already installed
    ...
    select_Platform_Binary_For_Version(${package} ${RES_VERSION} "${available_with_platform}" RES_PLATFORM)
    if(RES_PLATFORM)
...
if(NOT ALL_VERSIONS)
	message("[PID] ERROR : no adequate version found for wrapper of external package ${package} !! No version of ${package} seems to be buidable with current build process constraint.")
  restore_Repository_Context(${package} TRUE ${CURRENT_COMMIT} ${SAVED_CONTENT})
  return()
endif()
...
set(explaining_constraint)
if(is_exact)
  set(explaining_constraint "with exact minimum version ${min_version}")
  select_Exact_External_Version(RES_VERSION ${min_version} "${ALL_VERSIONS}")
else()
  set(explaining_constraint "with minimum version ${min_version}")
	select_Best_External_Version(RES_VERSION ${package} ${min_version} "${ALL_VERSIONS}")
endif()

if(NOT RES_VERSION)
	message("[PID] WARNING : no adequate version found for wrapper of external package ${package} ${explaining_constraint}!! Maybe this is due to a malformed package (contact the administrator of this package).")
  restore_Repository_Context(${package} TRUE ${CURRENT_COMMIT} ${SAVED_CONTENT})
  return()
endif()
list(FIND already_installed_versions ${RES_VERSION} INDEX)
...
if(INDEX EQUAL -1) # selected version not found in versions already installed
	check_Package_Version_State_In_Current_Process(${package} ${RES_VERSION} RES)
	if(RES STREQUAL "UNKNOWN") # this package version has not been build since beginning of the current process
		select_Platform_Binary_For_Version(${package} ${RES_VERSION} "${available_with_platform}" TARGET_PLATFORM)
...
select_Platform_Binary_For_Version(${package} "${version}" "${available_with_platform}" TARGET_PLATFORM)
if(NOT TARGET_PLATFORM)
...
if(res STREQUAL NOTFOUND)#file not found
  ...
  message("[PID] INTERNAL ERROR : no install manifest found for binary ${package} version ${version}. This is an internal bug that should never appear !")
...
if(AVOID_SYSTEM_DEPENDENCIES)#avoid forced if required
  list(FIND AVOID_SYSTEM_DEPENDENCIES ${package} INDEX)
  if(NOT INDEX EQUAL -1)
    set(try_force FALSE)
  endif()
endif()
