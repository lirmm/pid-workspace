  ...
  while(argument_couples)
    ...
    foreach(iter RANGE 0 ${last_elem_index_mem} 2)
      ...
      if(constraint_name STREQUAL arg_name)#argument with same value found
        ...
        foreach(arg_val IN LISTS ARG_VAL_LIST)#Note: for list we siply check if the value to check belongs to the list, we do not check strict equality between lists
          list(FIND CONSTRAINT_VAL_LIST ${arg_val} INDEX)
          if(INDEX EQUAL -1)
...
function(get_Deployment_Unit_Reference_Info path_to_repo REF_EXISTS RES_URL RES_PUBLIC_URL)
  ...
  if(NOT PATH_TO_FILE)
    ...
    if(GENERATE_REF)
      set(LOCAL_REF_PATH ${CMAKE_BINARY_DIR}/share/Refer${REF_PREFIX}${DU_NAME}.cmake)
      if(EXISTS ${LOCAL_REF_PATH})
        get_Path_To_All_Deployment_Unit_References_Publishing_Contribution_Spaces(ALL_PUBLISHING_CS ${DU_NAME} "")
        foreach(cs_path IN LISTS ALL_PUBLISHING_CS)
          file(COPY ${LOCAL_REF_PATH}
...
function(hard_Clean_Build_Folder path_to_folder)
  ...
  foreach(entry IN LISTS build_content)
    if(NOT entry MATCHES "^.*\\.gitignore$")
...
#     :package: the name of the target package.
#
function(reconfigure_Package_Build package release_only)
set(TARGET_BUILD_FOLDER ${WORKSPACE_DIR}/packages/${package}/build)
if(release_only)
  set(DO_ON=ON)
else()
...
execute_process(COMMAND ${CMAKE_COMMAND} -DBUILD_RELEASE_ONLY:BOOL=${DO_ON} .. WORKING_DIRECTORY ${TARGET_BUILD_FOLDER} )
...
# .. ifmode:: internal
#
#  .. |version_requirement_satisfied| replace:: ``version_requirement_satisfied``
#  .. _version_requirement_satisfied:
#
#  version_requirement_satisfied
#  -------------------------------
#
#   .. command:: version_requirement_satisfied(SATISFIED package version_requirement list_of_existing)
#
#    Check whether version requirement of a package has been released
#
#     :package: the name of the dependency.
#     :version_requirement: the required version of the depenency.
#     :list_of_existing: the rlist of already released dependencies 
#
#     :SATISFIED: the output variable is TRUE if a version matching the requirement is released.
#
function(version_requirement_satisfied SATISFIED package version_requirement list_of_existing)
  get_Version_String_Numbers("${version_requirement}" MAJOR MINOR PATCH)
  if(NOT DEFINED MINOR)
    foreach(existing IN LISTS list_of_existing)
      get_Version_String_Numbers("${existing}" EXIST_MAJOR EXIST_MINOR EXIST_PATCH)
      if(MAJOR EQUAL EXIST_MAJOR)
        set(${SATISFIED} TRUE PARENT_SCOPE)
        return()      
      endif()
    endforeach()
  elseif(NOT DEFINED PATCH)
    foreach(existing IN LISTS list_of_existing)
      get_Version_String_Numbers("${existing}" EXIST_MAJOR EXIST_MINOR EXIST_PATCH)
      if(MAJOR EQUAL EXIST_MAJOR AND MINOR EQUAL EXIST_MINOR)
        set(${SATISFIED} TRUE PARENT_SCOPE)
        return()      
      endif()
    endforeach()
  else()#complete version specification
    list(FIND list_of_existing ${version_requirement} INDEX)
    if(NOT INDEX EQUAL -1)# the version of dependency has not been released yet
      set(${SATISFIED} TRUE PARENT_SCOPE)
      return()
    endif()
  endif()
  set(${SATISFIED} FALSE PARENT_SCOPE)
endfunction(version_requirement_satisfied)
...
#
#     :package: the name of the target package.
#     :release_only: the name of the target package.
#
#     :BAD_DEPS: the output variable containing the list of packages whose version in use are not released.
#
function(check_For_Dependencies_Version BAD_DEPS package release_only)
...
OR (NOT EXISTS ${WORKSPACE_DIR}/packages/${package}/build/debug/share/Dep${package}.cmake AND NOT release_only))
  if(EXISTS ${WORKSPACE_DIR}/packages/${package})# the package is not binary only
    #simply reconfigure to get the dependencies
    reconfigure_Package_Build(${package} ${release_only})
    if(NOT EXISTS ${WORKSPACE_DIR}/packages/${package}/build/release/share/Dep${package}.cmake
...
if(NOT release_only)
  ...
  # now check that target dependencies version are released
  #debug
  foreach(dep IN LISTS TARGET_NATIVE_DEPENDENCIES_DEBUG)
    if(EXISTS ${WORKSPACE_DIR}/packages/${dep})#checking that the user may use a version generated by a source package
      ...
      if(TARGET_NATIVE_DEPENDENCY_${dep}_VERSION_DEBUG)
        version_requirement_satisfied(IS_OK ${dep} ${TARGET_NATIVE_DEPENDENCY_${dep}_VERSION_DEBUG} "${VERSION_NUMBERS}")
        if(NOT IS_OK)# the version of dependency has not been released yet
...
foreach(dep IN LISTS TARGET_NATIVE_DEPENDENCIES)
	if(EXISTS ${WORKSPACE_DIR}/packages/${dep})#checking that the user may use a version generated by a source package
      ...
      version_requirement_satisfied(IS_OK ${dep} ${TARGET_NATIVE_DEPENDENCY_${dep}_VERSION} "${VERSION_NUMBERS}")
			if(NOT IS_OK)# the version of dependency has not been released yet
...
function(enforce_Standard_Install_Dirs install_directory)
  if(EXISTS ${install_directory}/lib64)
    file(RENAME ${install_directory}/lib64 ${install_directory}/lib)
    create_Symlink(${install_directory}/lib ${install_directory}/lib64)
  elseif(EXISTS ${install_directory}/lib32)
    file(RENAME ${install_directory}/lib32 ${install_directory}/lib)
    create_Symlink(${install_directory}/lib ${install_directory}/lib32)
  endif()
  if(EXISTS ${install_directory}/bin64)
    file(RENAME ${install_directory}/bin64 ${install_directory}/bin)
    create_Symlink(${install_directory}/bin ${install_directory}/bin64)
  elseif(EXISTS ${install_directory}/bin32)
    file(RENAME ${install_directory}/bin32 ${install_directory}/bin)
    create_Symlink(${install_directory}/bin ${install_directory}/bin32)
  endif()
...
########################################################################################################
############ functions common to various CMake scripting APIs (e.g. wrapper script and system evaulation)
########################################################################################################
...
if(GET_TARGET_PLATFORM_INFO_DISTRIB_VERSION)
		set(OK TRUE)
		set(${GET_TARGET_PLATFORM_INFO_DISTRIB_VERSION} ${CURRENT_DISTRIBUTION_VERSION} PARENT_SCOPE)
endif()
