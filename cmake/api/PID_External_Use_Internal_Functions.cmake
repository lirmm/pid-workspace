...
function(reset_Local_Components_Info path mode)
  ...
  if(WORKSPACE_MODE) # configuration as already ran
    ...
    endforeach()
    unset(${PROJECT_NAME}_PID_PACKAGES CACHE)#reset list of packages
  ...
  foreach(comp IN LISTS DECLARED_LOCAL_COMPONENTS)
    unset(${comp}_LOCAL_DEPENDENCIES CACHE)
    unset(${comp}_PID_DEPENDENCIES CACHE)
    unset(${comp}_RUNTIME_RESOURCES CACHE)
  endforeach()
  unset(DECLARED_LOCAL_COMPONENTS CACHE)

  foreach(config IN LISTS DECLARED_SYSTEM_DEPENDENCIES)
    unset(${config}_VERSION_STRING CACHE)
    unset(${config}_REQUIRED_VERSION_EXACT CACHE)
    unset(${config}_REQUIRED_VERSION_SYSTEM CACHE)
  endforeach()
  unset(DECLARED_SYSTEM_DEPENDENCIES CACHE)
