...
macro(reset_Profiles)
  ...
  fill_String_From_List(res_limit LIMITED_JOBS_PACKAGES " ")
  fill_String_From_List(res_forced FORCED_SYSTEM_DEPENDENCIES " ")
  fill_String_From_List(res_avoid AVOID_SYSTEM_DEPENDENCIES " ")
  set(args -DWORKSPACE_DIR=${WORKSPACE_DIR} -DIN_CI_PROCESS=${IN_CI_PROCESS} -DPACKAGE_BINARY_INSTALL_DIR=${PACKAGE_BINARY_INSTALL_DIR} -DLIMITED_JOBS_PACKAGES="${res_limit}" -DFORCED_SYSTEM_DEPENDENCIES="${res_forced}" -DAVOID_SYSTEM_DEPENDENCIES="${res_avoid}")
  ...
  if(EXISTS ${dir}/PID_Toolchain.cmake)
    ...
    if(EXISTS ${dir}/Workspace_Build_Info.cmake)
      ...
      message(FATAL_ERROR "[PID] CRITICAL ERROR: No platform information generated by PID workspace. This is a severe issue due to a crash in build environment detection, please read previous outputs to understand the cause of the problem. If nothing appear use ADDITIONAL_DEBUG_INFO=ON when configuring the workspace.")
    endif()
  endif()
...
function(check_Language_Configuration_With_Arguments CHECK_OK BINARY_CONSTRAINTS CONFIGS_TO_CHECK lang_name lang_args mode)
  ...
  check_Configuration_Temporary_Optimization_Variables(CHECK_ALREADY_MADE CHECK_SUCCESS BIN_CONSTRAINTS UNUSED_INDEX ${lang_name} ${lang_args} ${mode})
...
function(check_Language_Toolset_Configuration_With_Arguments CHECK_OK toolset_expr langs toolset_name toolset_args_var mode)
  ...
  check_Configuration_Temporary_Optimization_Variables(CHECK_ALREADY_MADE CHECK_SUCCESS RES_CONSTRAINTS UNUSED_INDEX ${toolset_name} ${toolset_args_var} ${mode})
  if(CHECK_ALREADY_MADE)#same check has already been made, we want to avoid redoing them unecessarily
    set(${CHECK_OK} ${CHECK_SUCCESS} PARENT_SCOPE)
    return()
  endif()
...
function(check_Extra_Tool_Configuration_With_Arguments ENV_EXPR CONFIGS tool_expr tool tool_args_var mode)
  ...
  check_Configuration_Temporary_Optimization_Variables(CHECK_ALREADY_MADE CHECK_SUCCESS RES_CONSTRAINTS UNUSED_INDEX ${tool} ${tool_args_var} ${mode})
  if(CHECK_ALREADY_MADE)#same check has already been made, we want to avoid redoing them unecessarily
