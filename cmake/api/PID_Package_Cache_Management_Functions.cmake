...
get_Job_Count_For(${PROJECT_NAME} job_count)
if(job_count GREATER 0)
  set(PARALLEL_JOBS_FLAG "-j${job_count}" CACHE INTERNAL "")
endif()
...
function(set_Global_Options_From_Mode_Specific)
  ...
  if(NOT BUILD_RELEASE_ONLY)
    ...
    if(NOT EXISTS ${CMAKE_BINARY_DIR}/optionsDEBUG.txt)
      if(ADDITIONAL_DEBUG_INFO)
        message("[PID] INFO: file ${CMAKE_BINARY_DIR}/optionsDEBUG.txt not found, initial configuration of Debug build has been forced stopped. Skipping...")
      endif()
      return()
    endif()
  ...
  if(NOT EXISTS ${CMAKE_BINARY_DIR}/optionsRELEASE.txt)
    if(ADDITIONAL_DEBUG_INFO)
      message("[PID] INFO: file ${CMAKE_BINARY_DIR}/optionsRELEASE.txt not found, initial configuration of Release build has been forced stopped. Skipping...")
    endif()
    return()
  endif()
...
function(set_Install_Cache_Variables)
  ...
  set(CMAKE_INSTALL_PREFIX ${${PROJECT_NAME}_INSTALL_PATH}/${${PROJECT_NAME}_DEPLOY_PATH}  CACHE INTERNAL "" FORCE)
	set(${PROJECT_NAME}_INSTALL_LIB_PATH lib CACHE INTERNAL "")
	set(${PROJECT_NAME}_INSTALL_AR_PATH lib CACHE INTERNAL "")
	set(${PROJECT_NAME}_INSTALL_HEADERS_PATH include CACHE INTERNAL "")
	set(${PROJECT_NAME}_INSTALL_SHARE_PATH share CACHE INTERNAL "")
	set(${PROJECT_NAME}_INSTALL_SCRIPT_PATH ${${PROJECT_NAME}_INSTALL_SHARE_PATH}/script CACHE INTERNAL "")
	set(${PROJECT_NAME}_INSTALL_BIN_PATH bin CACHE INTERNAL "")
	set(${PROJECT_NAME}_INSTALL_RPATH_DIR .rpath CACHE INTERNAL "")
...
set(${package}_${component}_C_STANDARD${VAR_SUFFIX} CACHE INTERNAL "")
set(${package}_${component}_CXX_STANDARD${VAR_SUFFIX} CACHE INTERNAL "")
set(${package}_${component}_C_MAX_STANDARD${VAR_SUFFIX} CACHE INTERNAL "")
set(${package}_${component}_CXX_MAX_STANDARD${VAR_SUFFIX} CACHE INTERNAL "")
...
if(external)
  ...
  init_dependency_version_selection_option(${dep_package} TRUE ${optional} "${possible_versions}" "${exact_versions}")
  ...
  init_dependency_version_selection_option(${dep_package} FALSE ${optional} "${possible_versions}" "${exact_versions}")
endif()
...
function(reset_Temporary_Optimization_Variables mode)
  ...
  foreach(config IN LISTS TEMP_CONFIGS${VAR_SUFFIX})
    set(checks_number ${TEMP_CONFIG_${config}_CHECKS${VAR_SUFFIX}})
    math(EXPR total_range "${checks_number}-1")
...
function(set_Configuration_Temporary_Optimization_Variables config mode test_ok call_constraints binary_constraints result_index)
  ...
  if(NOT DEFINED TEMP_CONFIG_${config}_CHECKS${VAR_SUFFIX})
    ...
    set(next_index ${TEMP_CONFIG_${config}_CHECKS${VAR_SUFFIX}})
  endif()
