...
function(write_Platform_Reporting_File file)
  ...
  if(CURRENT_PROFILE STREQUAL "default")
    ...
    set(prefix ${def_name}_${def_hash})
    if(${prefix}_ACTION_INFO)
      string(CONFIGURE "${${prefix}_ACTION_INFO}" configured_actions)
  ...
  if(Python_Language_AVAILABLE)
    ...
    if(CURRENT_PYTHON_PACKAGER)
      set(WORKSPACE_CONFIGURATION_DESCRIPTION "${WORKSPACE_CONFIGURATION_DESCRIPTION}, ${CURRENT_PYTHON_PACKAGER} packaging")
    endif()
  ...
  if(PROFILE_${CURRENT_PROFILE}_MORE_ENVIRONMENTS)
    ...
    foreach(add_env IN LISTS PROFILE_${CURRENT_PROFILE}_MORE_ENVIRONMENTS)
      hashcode_From_Expression(env_name env_hash "${add_env}")
      set(mess_str "  -  ${add_env}")
      if(${env_name}_${env_hash}_ACTION_INFO)
        string(CONFIGURE "${${env_name}_${env_hash}_ACTION_INFO}" configured_actions)
  ...
  load_Workspace_Info()
	if("${build_folder}" MATCHES "${PROJECT_NAME}/build$")
		if(TEMP_PLATFORM)
          ...
          OR (NOT TEMP_ASM_COMPILER STREQUAL CMAKE_ASM_COMPILER)
...
load_Profile_Info()
#loading the current platform configuration simply consists in including the config file generated by the workspace
include(${WORKSPACE_DIR}/build/${CURRENT_PROFILE}/Workspace_Info.cmake)#WARNING may overwrite important Toolchain related variables
if(EXISTS ${WORKSPACE_DIR}/build/${PROJECT_NAME}_PID_Toolchain.cmake)
  include(${WORKSPACE_DIR}/build/${PROJECT_NAME}_PID_Toolchain.cmake)
endif()
...
function(check_Package_Platform_Against_Current CHECK_OK package platform version)
  ...
  convert_Configuration_Expressions(${package} Release) #deal with configuration expressions formats
