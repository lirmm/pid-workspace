...
list(APPEND CMAKE_MODULE_PATH ${WORKSPACE_DIR}/cmake/references) # using common find modules of the workspace
...
elseif(DIR_NAME STREQUAL "build")
	...
	if(NOT CMAKE_VERSION VERSION_LESS 3.2.3) #version of CMake is >= 3.2.3 => BYPRODUCTS argument can be used to configure Ninja
		add_custom_target(checksources
				...
				COMMAND ${CMAKE_COMMAND} -DWORKSPACE_DIR=${WORKSPACE_DIR}
							 ...
							 -P ${WORKSPACE_DIR}/cmake/system/commands/Check_PID_Package_Modification.cmake
	...
	add_custom_target(update
		COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
						...
						-P ${WORKSPACE_DIR}/cmake/system/commands/Update_PID_Package.cmake
	...
	add_custom_target(integrate
		COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
						...
						-P ${WORKSPACE_DIR}/cmake/system/commands/Integrate_PID_Package.cmake
	...
	add_custom_target(sync-version
		COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
						-DTARGET_PACKAGE=${PROJECT_NAME}
						-P ${WORKSPACE_DIR}/cmake/system/commands/Synchronize_PID_Package_Version.cmake
	...
	add_custom_target(check-branch
		COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
						...
						-P ${WORKSPACE_DIR}/cmake/system/commands/Check_PID_Package_Branch.cmake
	...
	add_custom_target(check-repository
		COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
						-DTARGET_PACKAGE=${PROJECT_NAME}
						-P ${WORKSPACE_DIR}/cmake/system/commands/Check_PID_Package_Official_Repository.cmake
			...
			if(CLANG_FORMAT_EXE)
				add_custom_target(format
					COMMAND ${CMAKE_COMMAND} -DWORKSPACE_DIR=${WORKSPACE_DIR}
								 ...
								 -P ${WORKSPACE_DIR}/cmake/system/commands/Format_PID_Package_Sources.cmake
...
if(${PROJECT_NAME}_SITE_GIT_ADDRESS) #the publication of the static site is done within a lone static site
	...
	add_custom_target(site
		COMMAND ${CMAKE_COMMAND} 	-DWORKSPACE_DIR=${WORKSPACE_DIR}
			 ...
			 -P ${WORKSPACE_DIR}/cmake/system/commands/Build_PID_Site.cmake)
...
foreach(dep_pack IN LISTS ${PROJECT_NAME}_EXTERNAL_DEPENDENCIES${USE_MODE_SUFFIX})
	...
	if(MUST_BE_INSTALLED)
		...
		if(NOT INSTALL_OK)
			finish_Progress(${GLOBAL_PROGRESS_VAR})
			message(FATAL_ERROR "[PID] CRITICAL ERROR : impossible to install external package: ${dep_pack}. This bug is maybe due to bad referencing of this package. Please have a look in workspace and try to find ReferExternal${dep_pack}.cmake file in cmake/references folder.")
			return()
		endif()
...
if(${PROJECT_NAME}_DEPENDENCIES${USE_MODE_SUFFIX})
	...
	foreach(dep_pack IN LISTS ${PROJECT_NAME}_DEPENDENCIES${USE_MODE_SUFFIX})
		...
		if(MUST_BE_INSTALLED)
			...
			if(NOT INSTALL_OK)
				finish_Progress(${GLOBAL_PROGRESS_VAR})
				message(FATAL_ERROR "[PID] CRITICAL ERROR : impossible to install native package: ${dep_pack}. This bug is maybe due to bad referencing of this package. Please have a look in workspace and try to find Refer${dep_pack}.cmake file in cmake/references folder.")
				return()
			endif()
...
if(${CMAKE_BUILD_TYPE} MATCHES Release)
	...
	add_custom_target(referencing
		COMMAND ${CMAKE_COMMAND}
						...
						-P ${WORKSPACE_DIR}/cmake/system/commands/Reference_PID_Package.cmake
	...
	if(	DEFINED ${PROJECT_NAME}_LICENSE
		...
		add_custom_target(licensing
			COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
							...
							-DSOURCE_DIR=${CMAKE_SOURCE_DIR}
							-DBINARY_DIR=${CMAKE_BINARY_DIR}
							-P ${WORKSPACE_DIR}/cmake/system/commands/Licensing_PID_Package_Files.cmake
			VERBATIM
		)
	endif()
...
add_custom_target(list_dependencies
	COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
					...
					-P ${WORKSPACE_DIR}/cmake/system/commands/Listing_PID_Package_Dependencies.cmake
)
		...
		if(SIZE EQUAL 1)
			add_custom_target(build-dependencies
				COMMAND ${CMAKE_COMMAND}	-DWORKSPACE_DIR=${WORKSPACE_DIR}
								...
								-P ${WORKSPACE_DIR}/cmake/system/commands/Build_PID_Package_Dependencies.cmake
