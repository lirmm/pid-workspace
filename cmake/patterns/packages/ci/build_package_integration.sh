...
if [ "$PACKAGE_HAS_LIBRARIES" = true ] ; then
  if [ "$PACKAGE_HAS_TESTS" = true ] ; then
     if [ "$PACKAGE_HAS_EXAMPLES" = true ] ; then
        cmake -DBUILD_RELEASE_ONLY=OFF -DREQUIRED_PACKAGES_AUTOMATIC_DOWNLOAD=ON -DADDITIONAL_DEBUG_INFO=ON -DBUILD_AND_RUN_TESTS=ON -DENABLE_SANITIZERS=ON -DRUN_TESTS_IN_DEBUG=ON -DBUILD_COVERAGE_REPORT=ON -DENABLE_PARALLEL_BUILD=ON -DBUILD_EXAMPLES=ON -DBUILD_API_DOC=OFF -DBUILD_STATIC_CODE_CHECKING_REPORT=ON -DWORKSPACE_DIR="../binaries/pid-workspace" ..
     else
        cmake -DBUILD_RELEASE_ONLY=OFF -DREQUIRED_PACKAGES_AUTOMATIC_DOWNLOAD=ON -DADDITIONAL_DEBUG_INFO=ON -DBUILD_AND_RUN_TESTS=ON -DENABLE_SANITIZERS=ON -DRUN_TESTS_IN_DEBUG=ON -DBUILD_COVERAGE_REPORT=ON -DENABLE_PARALLEL_BUILD=ON -DBUILD_EXAMPLES=OFF -DBUILD_API_DOC=OFF -DBUILD_STATIC_CODE_CHECKING_REPORT=ON -DWORKSPACE_DIR="../binaries/pid-workspace" ..
     fi
  else
      if [ "$PACKAGE_HAS_EXAMPLES" = true ] ; then
    ...
    if [ "$PACKAGE_HAS_TESTS" = true ] ; then
      cmake -DBUILD_RELEASE_ONLY=OFF -DREQUIRED_PACKAGES_AUTOMATIC_DOWNLOAD=ON -DBUILD_AND_RUN_TESTS=ON -DENABLE_SANITIZERS=ON -DRUN_TESTS_IN_DEBUG=ON -DBUILD_COVERAGE_REPORT=ON -DENABLE_PARALLEL_BUILD=ON -DBUILD_EXAMPLES=OFF -DBUILD_API_DOC=OFF -DBUILD_STATIC_CODE_CHECKING_REPORT=ON -DADDITIONAL_DEBUG_INFO=OFF -DWORKSPACE_DIR="../binaries/pid-workspace" ..
    else
