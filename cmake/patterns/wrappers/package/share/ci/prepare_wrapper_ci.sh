...
if [ ! -d "./binaries/pid-workspace" ]; then
  cd binaries && git clone git@gite.lirmm.fr:pid/pid-workspace.git && cd pid-workspace/build && cmake -DIN_CI_PROCESS=ON -DFORCE_CONTRIBUTION_SPACE="$PACKAGE_CONTRIBUTION_SPACES" .. && cd ../../..
else
