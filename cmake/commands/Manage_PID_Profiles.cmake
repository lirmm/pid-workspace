...
if(NOT TARGET_COMMAND MATCHES "^${cmd_list}$")
	message(FATAL_ERROR "[PID] ERROR : bad command \"${TARGET_COMMAND}\" used for profiles management. Allowed commands are: ${cmd_list}")
endif()
if(NOT TARGET_COMMAND MATCHES "ls|reset")#except when listing profiles or resetting to defaultprofile, a profile name must be given
	if(NOT TARGET_PROFILE)
		if(TARGET_COMMAND MATCHES "add|rm|load")#add and rm commands apply by default to current profile
			...
			message(FATAL_ERROR "[PID] ERROR : when using the command ${TARGET_COMMAND}: you must given the name of target profile using \"profile\" argument")
		endif()
	endif()

	if(TARGET_COMMAND MATCHES "mk|add|rm")
		if(NOT TARGET_ENVIRONMENT)
			message(FATAL_ERROR "[PID] ERROR : when using the command ${TARGET_COMMAND}: you must give the name of environment using \"env\" argument")
		endif()
	endif()
	if(TARGET_COMMAND STREQUAL "mk")
		#more checks: when a profile is created target platform must be more or less precisely specified
		if(TARGET_PLATFORM)
...
if(TARGET_COMMAND STREQUAL "ls")
	message("[PID] INFO : current profile is ${CURRENT_PROFILE}")
	message("[PID] INFO : all available profiles: ")
	foreach(profile IN LISTS PROFILES)
		message(" -[PID] INFO : ${profile}: based on ${PROFILE_${profile}_DEFAULT_ENVIRONMENT} environment")
		if(PROFILE_${profile}_TARGET_SYSROOT)
			message("[PID] INFO :     + sysroot: ${PROFILE_${profile}_TARGET_SYSROOT}")
		endif()
		if(PROFILE_${profile}_TARGET_STAGING)
			message("[PID] INFO :     + staging: ${PROFILE_${profile}_TARGET_STAGING}")
		endif()
		if(PROFILE_${profile}_TARGET_INSTANCE)
			message("[PID] INFO :     + instance name: ${PROFILE_${profile}_TARGET_INSTANCE}")
		endif()
		if(PROFILE_${profile}_TARGET_PLATFORM_OS
			...
			set(mess_str "[PID] INFO :     + target platform:")
			if(PROFILE_${profile}_TARGET_PLATFORM_OS)
		...
		if(PROFILE_${profile}_TARGET_DISTRIBUTION)
			if(PROFILE_${profile}_TARGET_DISTRIBUTION_VERSION)
				message("[PID] INFO :   + target distribution: ${PROFILE_${profile}_TARGET_DISTRIBUTION} ${PROFILE_${profile}_TARGET_DISTRIBUTION_VERSION}")
			else()
				message("[PID] INFO :   + target distribution: ${PROFILE_${profile}_TARGET_DISTRIBUTION}")
			endif()
		endif()
		if(PROFILE_${profile}_MORE_ENVIRONMENTS)
			fill_String_From_List(RES_STR PROFILE_${profile}_MORE_ENVIRONMENTS " ")
			message("[PID] INFO :   + additionnal environments: ${RES_STR}")
		endif()
  endforeach()
...
elseif(TARGET_COMMAND STREQUAL "del")
	...
	if(INDEX EQUAL -1)#profile does not exist
		message(FATAL_ERROR "[PID] ERROR : target profile ${TARGET_PROFILE} does not exist, cannot delete it.")
	endif()
...
elseif(TARGET_COMMAND STREQUAL "load")
	list(FIND PROFILES ${TARGET_PROFILE} INDEX)
	if(INDEX EQUAL -1)#profile does not exist
		message(FATAL_ERROR "[PID] ERROR : target profile ${TARGET_PROFILE} does not exist, cannot load it.")
	endif()
...
elseif(TARGET_COMMAND STREQUAL "mk")
	...
	if(NOT INDEX EQUAL -1)#profile does not exist
		message(FATAL_ERROR "[PID] ERROR : target profile ${TARGET_PROFILE} already exists, cannot create a profile with same name*.")
	endif()
	set(args )
	if(TARGET_SYSROOT)
	...
	if(TARGET_OS)
		list(APPEND args OS ${TARGET_OS})
	endif()
	if(TARGET_PROC_ARCH)
		list(APPEND args PROC_ARCH ${TARGET_PROC_ARCH})
	endif()
	if(TARGET_PROC_TYPE)
		list(APPEND args PROC_TYPE ${TARGET_PROC_TYPE})
	endif()
	if(TARGET_ABI)
		list(APPEND args ABI ${TARGET_ABI})
	endif()
...
elseif(TARGET_COMMAND STREQUAL "add")
	list(FIND PROFILES ${TARGET_PROFILE} INDEX)
	if(INDEX EQUAL -1)#profile does not exist
		message(FATAL_ERROR "[PID] ERROR : target profile ${TARGET_PROFILE} does not exist, cannot add an additionnal environment to it.")
	endif()
...
elseif(TARGET_COMMAND STREQUAL "rm")
	list(FIND PROFILES ${TARGET_PROFILE} INDEX)
	if(INDEX EQUAL -1)#profile does not exist
		message(FATAL_ERROR "[PID] ERROR : target profile ${TARGET_PROFILE} does not exist, cannot remove an additionnal environment from it.")
	endif()
	remove_Additional_Environment(SUCCESS ${TARGET_PROFILE} ${TARGET_ENVIRONMENT})
	if(NOT SUCCESS)
		message(FATAL_ERROR "[PID] ERROR : target profile ${TARGET_PROFILE} does not have an additionnal environment named ${TARGET_ENVIRONMENT}, cannot remove it.")
	endif()
