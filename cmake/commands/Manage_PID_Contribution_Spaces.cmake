...
if(NOT TARGET_COMMAND)
  message("[PID] ERROR : no command defined when managing contribution spaces. Use cmd argument with a value chosen among ${cmd_list}.")
  return()
elseif(NOT TARGET_COMMAND MATCHES "^${cmd_list}$")
  message("[PID] ERROR :when managing contribution spaces, command ${TARGET_COMMAND} is unknown. Use cmd argument with a value chosen among ${cmd_list}.")
  return()
elseif(NOT TARGET_COMMAND MATCHES "^ls|reset$")
  if(NOT TARGET_CS)
    message("[PID] ERROR :when managing contribution spaces, no target contribution space defined (use space argument).")
    return()
  endif()
  if(TARGET_COMMAND STREQUAL "add")
    if(NOT UPDATE_URL)
      message("[PID] ERROR :when managing contribution spaces, command add requires argument update (url where to get last updates of the contribution space) to be defined.")
      return()
    endif()
  elseif(TARGET_COMMAND STREQUAL "churl")
    if(NOT PUBLISH_URL)
      message("[PID] ERROR :when managing contribution spaces, command churl requires argument publish (url used to publish updates) to be defined.")
      return()
    endif()
  elseif(TARGET_COMMAND MATCHES "^move|copy|delete$")
    if(NOT CONTENT_TO_OPERATE)
      message("[PID] ERROR :when managing contribution spaces, command ${TARGET_COMMAND} requires argument content to be defined.")
      return()
    endif()
    if(TARGET_COMMAND MATCHES "^move|copy$")
      if(NOT SOURCE_CS)
        message("[PID] ERROR :when managing contribution spaces, command ${TARGET_COMMAND} requires argument from to be defined.")
        return()
      endif()
    endif()
  endif()
...
if(NOT SUCCESS
  ...
  message("[PID] ERROR : no contribution space in use, please configure again your workspace.")
  return()
endif()
...
elseif(TARGET_COMMAND STREQUAL "add")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(FOUND)
    message("[PID] ERROR : cannot add ${TARGET_CS} to contribution spaces since this contribution space is already used.")
    return()
  endif()
...
elseif(TARGET_COMMAND STREQUAL "churl")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot change url of ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
  #set the publish URL
...
elseif(TARGET_COMMAND MATCHES "^prio_max|prio_min$")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot change priority of ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
  #set the publish URL
  set(temp_list ${CONTRIBUTION_SPACES})
...
elseif(TARGET_COMMAND STREQUAL "publish")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot publish content from ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
...
elseif(TARGET_COMMAND STREQUAL "update")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot update content of ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
...
elseif(TARGET_COMMAND STREQUAL "move")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot move references in ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
  find_In_CS_List(FOUND ${SOURCE_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot move references from ${SOURCE_CS} because this contribution space is not used.")
    return()
  endif()
  get_Path_To_Contribution_Space(SOURCE_PATH ${SOURCE_CS})
  get_Path_To_Contribution_Space(TARGET_PATH ${TARGET_CS})
  get_All_Matching_Contributions(LICENSE REFERENCE FIND FORMAT CONFIGURATION PLUGIN ${SOURCE_CS} ${CONTENT_TO_OPERATE})
  if(LICENSE)
  ...
  if(CONFIGURATION)
    if(NOT EXISTS ${TARGET_PATH}/configurations)
      file(MAKE_DIRECTORY ${TARGET_PATH}/configurations)
    endif()
    file(RENAME ${SOURCE_PATH}/configurations/${CONFIGURATION} ${TARGET_PATH}/configurations/${CONFIGURATION})
  endif()
  if(PLUGIN)
...
elseif(TARGET_COMMAND STREQUAL "copy")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot copy contributions in ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
  find_In_CS_List(FOUND ${SOURCE_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot copy contributions from ${SOURCE_CS} because this contribution space is not used.")
    return()
  endif()
  get_Path_To_Contribution_Space(SOURCE_PATH ${SOURCE_CS})
  get_Path_To_Contribution_Space(TARGET_PATH ${TARGET_CS})
  ...
  if(CONFIGURATION)
    ...
    file(COPY ${SOURCE_PATH}/configurations/${CONFIGURATION} DESTINATION ${TARGET_PATH}/configurations)
  endif()
  if(PLUGIN)
    if(NOT EXISTS ${TARGET_PATH}/plugins)
      file(MAKE_DIRECTORY ${TARGET_PATH}/plugins)
...
elseif(TARGET_COMMAND STREQUAL "delete")
  find_In_CS_List(FOUND ${TARGET_CS})
  if(NOT FOUND)
    message("[PID] ERROR : cannot delete contributions in ${TARGET_CS} because this contribution space is not used.")
    return()
  endif()
  get_Path_To_Contribution_Space(TARGET_PATH ${TARGET_CS})
  get_All_Matching_Contributions(LICENSE REFERENCE FIND FORMAT CONFIGURATION PLUGIN ${TARGET_PATH} ${CONTENT_TO_OPERATE})
  if(LICENSE)
  ...
  if(CONFIGURATION)
    file(REMOVE_RECURSE ${TARGET_PATH}/configurations/${CONFIGURATION})
  endif()
  if(PLUGIN)
