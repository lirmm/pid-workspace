...
elseif(NOT TARGET_COMMAND MATCHES "^ls|reset|find|add$")
  if(NOT TARGET_CS)
...
elseif(TARGET_COMMAND STREQUAL "add")
  if(UPDATE_URL)#by default use the update URL as this is the one that is supposed to be the most widely accessible (typically https rather than ssh)
    set(used_url ${UPDATE_URL})
  else()
    set(used_url ${PUBLISH_URL})#in this cas it is usefull only for private contribution space the user has access to
  endif()
  if(TARGET_CS)
    find_In_CS_List(FOUND ${TARGET_CS})
    if(FOUND)
    ...
    #perform check to ensure user defined names and adresses are OK
    if(EXISTS ${PATH_TO_REPO}/contribution_space_init.cmake)
      include(${PATH_TO_REPO}/contribution_space_init.cmake)
      if(NOT CONTRIBUTION_SPACE_OFFICIAL_NAME OR NOT CONTRIBUTION_SPACE_OFFICIAL_UPDATE)
        message(FATAL_ERROR "[PID] ERROR: cannot add contribution space with repository address ${used_url} because this space has not been CORRECTLY initialized. Please ask the manager to solve the problem by correcting the contribution_space_init.cmake file contained in this repository.")
      endif()
      if(NOT TARGET_CS STREQUAL CONTRIBUTION_SPACE_OFFICIAL_NAME)
        message(FATAL_ERROR "[PID] ERROR: cannot add contribution space ${TARGET_CS} with repository address ${used_url} because this space has been initialized with another name: ${CONTRIBUTION_SPACE_OFFICIAL_NAME}.")
      endif()
      if(UPDATE_URL AND NOT UPDATE_URL STREQUAL CONTRIBUTION_SPACE_OFFICIAL_UPDATE)
        message(FATAL_ERROR "[PID] ERROR: cannot add contribution space ${TARGET_CS} with repository address ${used_url} because this space has not been initialized with a different update address: ${CONTRIBUTION_SPACE_OFFICIAL_UPDATE}.")
      endif()
    endif()
  else()
    #specific action to perform to check and set contribution space name
    # 1) clone the space repository
    #need to manage specific situation of a request to add a new contribution space specified without name
    clone_Contribution_Space_Repository(PATH_TO_REPO ${used_url})
    if(NOT PATH_TO_REPO)
      message(FATAL_ERROR "[PID] ERROR: cannot clone contribution space from repository address ${used_url}")
    endif()
    if(NOT EXISTS ${PATH_TO_REPO}/contribution_space_init.cmake)
      message(FATAL_ERROR "[PID] ERROR: cannot add contribution space with repository address ${used_url} because this space has not been initialized. Please rerun the add command using the `space` argument to set the name of contribition, this will initialize it.")
    endif()
    include(${PATH_TO_REPO}/contribution_space_init.cmake)
    if(NOT CONTRIBUTION_SPACE_OFFICIAL_NAME OR NOT CONTRIBUTION_SPACE_OFFICIAL_UPDATE)
      message(FATAL_ERROR "[PID] ERROR: cannot add contribution space with repository address ${used_url} because this space has not been CORRECTLY initialized. Please ask the manager to solve the problem by correcting the contribution_space_init.cmake file contained in this repository.")
    endif()
    set(TARGET_CS ${CONTRIBUTION_SPACE_OFFICIAL_NAME} CACHE INTERNAL "")
    if(NOT UPDATE_URL)
      set(UPDATE_URL ${CONTRIBUTION_SPACE_OFFICIAL_UPDATE} CACHE INTERNAL "")
    elseif(NOT UPDATE_URL STREQUAL CONTRIBUTION_SPACE_OFFICIAL_UPDATE)
      message(FATAL_ERROR "[PID] ERROR: cannot add contribution space with repository address ${used_url} because this space has not been initialized with a different update address: ${CONTRIBUTION_SPACE_OFFICIAL_UPDATE}.")
    endif()
    if(NOT PUBLISH_URL)#publish URL can be changed on demand OR let unchanged (for instance to directly use a fork of the official repository)
      set(PUBLISH_URL ${CONTRIBUTION_SPACE_OFFICIAL_PUBLISH} CACHE INTERNAL "")
    endif()
  #else no target space defined means that the contrbution space MUST contain description
  endif()
