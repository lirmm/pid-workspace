...
function (_MPI_check_compiler LANG QUERY_FLAG OUTPUT_VARIABLE RESULT_VARIABLE)
  if(DEFINED MPI_${LANG}_COMPILER_FLAGS)
    if(WIN32)
      separate_arguments(_MPI_COMPILER_WRAPPER_OPTIONS WINDOWS_COMMAND "${MPI_${LANG}_COMPILER_FLAGS}")
    else()
      separate_arguments(_MPI_COMPILER_WRAPPER_OPTIONS UNIX_COMMAND "${MPI_${LANG}_COMPILER_FLAGS}")
    endif()
  else()
    if(WIN32)
      separate_arguments(_MPI_COMPILER_WRAPPER_OPTIONS WINDOWS_COMMAND "${MPI_COMPILER_FLAGS}")
    else()
...
function (_MPI_interrogate_compiler LANG)
  ...
  if (NOT MPI_ALL_INCLUDE_PATHS)
    ...
    if(MPI_INCDIRS_COMPILER_RETURN)
      if(WIN32)
        separate_arguments(MPI_ALL_INCLUDE_PATHS WINDOWS_COMMAND "${MPI_INCDIRS_CMDLINE}")
      else()
  ...
  if (NOT MPI_ALL_LINK_PATHS)
    ...
    if(MPI_LIBDIRS_COMPILER_RETURN)
      if(WIN32)
        separate_arguments(MPI_ALL_LINK_PATHS WINDOWS_COMMAND "${MPI_LIBDIRS_CMDLINE}")
      else()
...
function(_MPI_try_staged_settings LANG MPI_TEST_FILE_NAME MODE RUN_BINARY)
  set(WORK_DIR "${CMAKE_BINARY_DIR}/FindMPI")
  set(SRC_DIR "${CMAKE_SOURCE_DIR}/FindMPI")
  set(BIN_FILE "${CMAKE_BINARY_DIR}/FindMPI/${MPI_TEST_FILE_NAME}_${LANG}.bin")
...
if(NOT MPI_IGNORE_LEGACY_VARIABLES)
  foreach (LANG IN ITEMS C CXX)
    ...
    if(MPI_${LANG}_COMPILE_FLAGS)
      if(WIN32)
        separate_arguments(MPI_SEPARATE_FLAGS WINDOWS_COMMAND "${MPI_${LANG}_COMPILE_FLAGS}")
      else()
...
foreach(LANG IN ITEMS C CXX Fortran)
  ...
  if(_MPI_FIND_${LANG})
    ...
    if(MPI_${LANG}_WORKS)
      ...
      set(MPI_BIN_FOLDER ${CMAKE_BINARY_DIR}/FindMPI)
