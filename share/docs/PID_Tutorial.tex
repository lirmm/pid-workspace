...
Other tools used, like the compiler/linker (e.g. gcc) used with cmake, or the ssh agent used with git, are supposed to be native (i.e. specific to the OS used) and so adapted by the user depending on the platform used.
...
\item the \textbf{install} folder contains packages' binaries installed by the user. Each of its direct sub directories is a folder where are installed package binary versions.
...
The install folder contains installed binary version(s) of any package used by a user.
