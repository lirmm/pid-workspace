...
%
%2.4.3 Libraries vs Executables
%One important concern when dealing with real-time software is to separate control flow aspects from pure computation aspects. Indeed, depending on the middleware used, the management of the control flow can vary according to the programming paradigm : threads/tasks/processes (called tasks in the remainder of the report) handling can be more or less constrained by available middleware services as well as process communication and synchronization (IPC). In robotic middlewares (e.g. ROS, OROCOS) reusable components are software tasks that use some computational code to perform a given objective for robot control. Since middlewares programming models vary, tasks are not reusable from one to another. If the computation code is “merged” into control flow management code it becomes not reusable from one context to another.
%
%The best way is so to separate computational and control-flow management code in source code :
%libraries (either shared, static or pure template header files) must contain computational code only : they cannot embeds code to deal with threads, processes, mutexes, semaphore or more generally any code dealing with processes synchronization and communication, shared memory management, scheduling, timing, etc.
%executables have to define control flow management code according to the selected middleware (or OS if no middleware is used). It should not define complex computation code but only used the one defined in libraries it uses and “plug” it with control flow management code.
%
%The benefit of this policy is to maximize the reuse of the computational code (algorithms) among different component programing paradigms.  Of course this policy does not apply when the set of libraries of a package are designed to define services of a middleware. 
%
%Important remark : discriminate when to put code in a library or in an executable is not always really easy. This is particularly the case for “drivers” (code that manage externally accessible devices). In this case the best way to do is to discriminate protocol specification from protocol runtime behavior (that sometimes requires multi-threading) :
%libraries implement the protocol specification : exchanged frames format encoding/decoding/ verification ; api for translation between high level requests and exchanged frames ; communication channel used.
%executables implement the protocol runtime behavior : they put in place multi-threading when necessary, interruption handling, timing behavior, according to the programming constraints defined in the “driver” library they use.
