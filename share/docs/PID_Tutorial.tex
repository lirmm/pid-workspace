...
Header libraries are libraries are not compiled to produce a binary library. They are just made of a set of header files. This kind of library used is often used for template libraries definitions, for instance the Boost library. Such a library is never used at link time (never linked to another library or executable using a linker) but only at compile time. The definition of a header lib should look like:
\begin{verbatim}
