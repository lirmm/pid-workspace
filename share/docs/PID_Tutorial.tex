...
The main idea is that a package is completely self-contained. It does not means it contains all code and artefacts it requires but it contains all information needed to satisfy its dependencies. In other words considering a given package, its installation process is done using these information and information contained in its dependent packages.
...
\subsubsection{Package Tooling}

For each development task in a package, a tool is used to achieve this task. To ease the portability of code, only cross platforms tools are used by default:
\begin{itemize}
...
\item the repository can be set "public" so that anyone is considered as a user. In this case the users group is not useful and can be let undefined.
\end{itemize}
...
Given a package, this package has an \textbf{official GIT repository} that is deployed on a given package server. This repository is \textit{official} in the sense that it centralizes information of the package and is public, which means that all concerned people (team, laboratory or more generally anyone registered in the server) can access to it. The access in itself is restricted with respects to roles:
\begin{itemize}
\item registered administrators have read/write access on the official package.
...
\subsection{Workspace and frameworks}
