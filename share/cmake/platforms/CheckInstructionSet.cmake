...
elseif(CURRENT_TYPE STREQUAL "arm")
  if(NOT CURRENT_ARCH EQUAL 64)
    if(NOT CURRENT_CXX_COMPILER STREQUAL "msvc")
      ...
      set(CPU_NEON_FLAGS_CONFLICT "-mfpu=[^ ]*")
      set(CPU_FP16_FLAGS "-mfpu=neon-fp16")
      set(CPU_FP16_FLAGS_CONFLICT "-mfpu=[^ ]*")
    endif()
  else()
    set(CPU_NEON_FLAGS "")
  endif()
...
function(check_compiler_optimization IS_SUPPORTED instruction_set)
  ...
  if(CPU_${instruction_set}_FLAGS OR CPU_${instruction_set}_TEST_FILE)#if there is either a test file or a known compiler option for that instruction set
    if(CPU_${instruction_set}_TEST_FILE)#there is a test file provided
      check_compiler_flag(FLAG_OK "" "${CPU_${instruction_set}_TEST_FILE}")
      #check_compiler_flag(FLAG_OK "${CPU_BASELINE_FLAGS}" "${CPU_${instruction_set}_TEST_FILE}")
      if(FLAG_OK)
        set(${IS_SUPPORTED} TRUE PARENT_SCOPE)
        return()
      elseif(CPU_${instruction_set}_FLAGS)
        check_compiler_flag(FLAG_OK "${CPU_${instruction_set}_FLAGS}" "${CPU_${instruction_set}_TEST_FILE}")
      endif()
    else()#no test file only use flags
