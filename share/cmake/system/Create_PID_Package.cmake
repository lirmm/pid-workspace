...
if(REQUIRED_PACKAGE)
	...
	if(NOT REQUIRED_STATUS STREQUAL NOTFOUND)
		message("[PID] ERROR : A package with the same name ${REQUIRED_PACKAGE} is already referenced in the workspace repository.")
		return()
	endif()
	if(EXISTS ${WORKSPACE_DIR}/packages/${REQUIRED_PACKAGE} AND IS_DIRECTORY ${WORKSPACE_DIR}/packages/${REQUIRED_PACKAGE})
		message("[PID] ERROR : A package with the same name ${REQUIRED_PACKAGE} already resides in the workspace filesystem.")
		return()
	endif()
	if(OPTIONAL_LICENSE)
		...
		if(REQUIRED_STATUS STREQUAL NOTFOUND)
			message("[PID] ERROR : License ${REQUIRED_LICENSE} does not refer to any known license in the workspace.")
			return()
		endif()
	endif()
	if(OPTIONNAL_GIT_URL)
		...
		if(NOT "${RES_NAME}" STREQUAL "${REQUIRED_PACKAGE}")
			message("[PID] ERROR : the git url of the repository (${REQUIRED_GIT_URL}) does not define a repository with same name than package ${REQUIRED_PACKAGE}.")
			return()
		endif()
	endif()
	...
	if(OPTIONNAL_GIT_URL)
		test_Remote_Initialized(${REQUIRED_PACKAGE} ${OPTIONNAL_GIT_URL} IS_INITIALIZED)#the package is initialized as soon as it has an integration branch (no really perfect but simple and preserve time and memory -> no need to fetch the repository, so no network delays)
		...
		message("[PID] INFO : new package ${REQUIRED_PACKAGE} has just been cloned from official remote ${OPTIONNAL_GIT_URL}.")
	else() #simply create the package locally
		create_PID_Package(${REQUIRED_PACKAGE} "${OPTIONAL_AUTHOR}" "${OPTIONAL_INSTITUTION}" "${OPTIONAL_LICENSE}")
		message("[PID] INFO : new package ${REQUIRED_PACKAGE} has just been created locally.") 
	endif()
	
else()
	message("[PID] ERROR : You must specify a name for the package to create using name=<name of package> argument.")
endif()
