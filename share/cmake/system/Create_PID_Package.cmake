...
if(TARGET_FRAMEWORK AND (NOT TARGET_FRAMEWORK STREQUAL ""))# a framework is created
	...
	if(OPTIONNAL_GIT_URL)
		...
		if(NOT "${RES_NAME}" STREQUAL "${REQUIRED_NAME}")
	...
	if(OPTIONNAL_GIT_URL)
		test_Framework_Initialized(${TARGET_FRAMEWORK} ${OPTIONNAL_GIT_URL} IS_INITIALIZED)#the framework is initialized as soon as it has a branch
...
elseif(TARGET_PACKAGE AND (NOT TARGET_PACKAGE STREQUAL ""))
	...
	# from here we are sure the request is well formed
	if(OPTIONNAL_GIT_URL)
		test_Remote_Initialized(${TARGET_PACKAGE} ${OPTIONNAL_GIT_URL} IS_INITIALIZED)#the package is initialized as soon as it has an integration branch (no really perfect but simple and preserve time and memory -> no need to fetch the repository, so no network delays)
		if(IS_INITIALIZED)#simply clone
