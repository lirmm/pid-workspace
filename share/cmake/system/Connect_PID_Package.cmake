...
if(REQUIRED_PACKAGE)
	if(NOT EXISTS ${WORKSPACE_DIR}/packages/${REQUIRED_PACKAGE} OR NOT IS_DIRECTORY ${WORKSPACE_DIR}/packages/${REQUIRED_PACKAGE})
		message("[PID] ERROR : Package ${REQUIRED_PACKAGE} to connect is not contained in the workspace.")
		return()
	endif()
	...
	elseif(NOT OFFICIAL_GIT_URL) # possible only if add an origin to a package that only gets an official (adding connection to a private version of the repository)
		...
		if(NOT ALREADY_CONNECTED)#the package must be connected to the official branch for this option to be valid
			message("[PID] ERROR : package ${REQUIRED_PACKAGE} is not connected to an official git repository. This indicates a package in a bad state. This must be solved using official=<address of the official repository> option.")
			return()
		endif()
		get_Repository_Name(RES_NAME ${ORIGIN_GIT_URL})	
		if(NOT "${RES_NAME}" STREQUAL "${REQUIRED_PACKAGE}")
			message("[PID] ERROR : the git url of the origin remote repository (${ORIGIN_GIT_URL}) does not define a repository with same name than package ${REQUIRED_PACKAGE}.")
			return()
		endif()
		...
		if(ALREADY_CONNECTED AND NOT (FORCED_RECONNECTION STREQUAL "true"))
			message("[PID] ERROR : package ${REQUIRED_PACKAGE} is already connected to an official git repository. Use the force=true option to force the reconnection.")
			return()
		endif()
		get_Repository_Name(RES_NAME ${OFFICIAL_GIT_URL})	
		if(NOT "${RES_NAME}" STREQUAL "${REQUIRED_PACKAGE}")
			message("[PID] ERROR : the git url of the official repository (${OFFICIAL_GIT_URL}) does not define a repository with same name than package ${REQUIRED_PACKAGE}.")
			return()
		endif()
		...
		if(ORIGIN_GIT_URL)#both official AND origin are set (sanity check)
			get_Repository_Name(RES_NAME ${ORIGIN_GIT_URL})	
			if(NOT "${RES_NAME}" STREQUAL "${REQUIRED_PACKAGE}")
				message("[PID] ERROR : the git url of the origin repository (${ORIGIN_GIT_URL}) does not define a repository with same name than package ${REQUIRED_PACKAGE}.")
				return()
			endif()
		endif()
		...
		message("[PID] INFO : Package ${REQUIRED_PACKAGE} has been connected with official remote  ${OFFICIAL_GIT_URL}.")
		if(ORIGIN_GIT_URL)
			add_Connection_To_PID_Package(${REQUIRED_PACKAGE} ${ORIGIN_GIT_URL})
			message("[PID] INFO : Package ${REQUIRED_PACKAGE} has been connected with origin remote  ${ORIGIN_GIT_URL}.")
		endif()
		
	endif()
	
else()
	message("[PID] ERROR : You must specify the name of the package to connect to a git repository using name=<name of package> argument.")
endif()
