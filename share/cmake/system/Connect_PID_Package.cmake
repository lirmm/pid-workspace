...
if(TARGET_FRAMEWORK AND (NOT TARGET_FRAMEWORK STREQUAL ""))# a framework is connected
	...
	if(NOT OFFICIAL_GIT_URL) # possible only if add an origin to a package that only gets an official (adding connection to a private version of the repository)
		...
		is_Framework_Connected(ALREADY_CONNECTED ${TARGET_FRAMEWORK} official)
		if(NOT ALREADY_CONNECTED)#the package must be connected to the official remote for this option to be valid
			message("[PID] ERROR : framework ${TARGET_FRAMEWORK} is not connected to an official git repository. This indicates a package in a bad state. This must be solved using official=<address of the official repository> option.")
			return()
		endif()
		get_Framework_Repository_Name(RES_NAME ${ORIGIN_GIT_URL})	
		if(NOT "${RES_NAME}" STREQUAL "${TARGET_FRAMEWORK}")
			message("[PID] ERROR : the git url of the origin remote repository (${ORIGIN_GIT_URL}) does not define a repository with same name than package ${TARGET_FRAMEWORK}.")
			return()
		endif()
		...
		add_Connection_To_PID_Framework(${TARGET_FRAMEWORK} ${ORIGIN_GIT_URL})
		...
		if(ALREADY_CONNECTED AND NOT (FORCED_RECONNECTION STREQUAL "true"))
			message("[PID] ERROR : framework ${TARGET_FRAMEWORK} is already connected to an official git repository. Use the force=true option to force the reconnection.")
			return()
		endif()
		get_Framework_Repository_Name(RES_NAME ${OFFICIAL_GIT_URL})	
		if(NOT "${RES_NAME}" STREQUAL "${TARGET_FRAMEWORK}")
			message("[PID] ERROR : the git url of the official repository (${OFFICIAL_GIT_URL}) does not define a repository with same name than framework ${TARGET_FRAMEWORK}.")
			return()
		endif()
		...
		if(ORIGIN_GIT_URL)#both official AND origin are set (sanity check)
			get_Framework_Repository_Name(RES_NAME ${ORIGIN_GIT_URL})	
			if(NOT "${RES_NAME}" STREQUAL "${TARGET_FRAMEWORK}")
				message("[PID] ERROR : the git url of the origin repository (${ORIGIN_GIT_URL}) does not define a repository with same name than framework ${TARGET_FRAMEWORK}.")
				return()
			endif()
		endif()

		if(FORCED_RECONNECTION STREQUAL "true")#changing the official repository
			connect_PID_Framework(${TARGET_FRAMEWORK} ${OFFICIAL_GIT_URL} FALSE)
		else()
			connect_PID_Framework(${TARGET_FRAMEWORK} ${OFFICIAL_GIT_URL} TRUE)
		endif()
		message("[PID] INFO : Framework ${TARGET_FRAMEWORK} has been connected with official remote  ${OFFICIAL_GIT_URL}.")
		if(ORIGIN_GIT_URL)
			add_Connection_To_PID_Package(${TARGET_FRAMEWORK} ${ORIGIN_GIT_URL})
			message("[PID] INFO : Framework ${TARGET_FRAMEWORK} has been connected with origin remote  ${ORIGIN_GIT_URL}.")
		endif()
	endif()
