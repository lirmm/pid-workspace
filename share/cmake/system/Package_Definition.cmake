...
if(NOT DECLARE_PID_PACKAGE_AUTHOR)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, an author name must be given using AUTHOR keyword.")
endif()
if(NOT DECLARE_PID_PACKAGE_YEAR)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a year or year interval must be given using YEAR keyword.")
endif()
if(NOT DECLARE_PID_PACKAGE_LICENSE)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a license type must be given using LICENSE keyword.")
endif()
if(NOT DECLARE_PID_PACKAGE_DESCRIPTION)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a (short) description of the package must be given using DESCRIPTION keyword.")
endif()

if(DECLARE_PID_PACKAGE_UNPARSED_ARGUMENTS)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, unknown arguments ${DECLARE_PID_PACKAGE_UNPARSED_ARGUMENTS}.")
endif()
...
elseif(${ARGC} EQUAL 2)
	...
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to input a major and a minor number, optionnaly you can set a patch version (considered as 0 if not set).")
endif()
...
if(ADD_PID_PACKAGE_REFERENCE_SOURCE)
	if(NOT ADD_PID_PACKAGE_REFERENCE_URL)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the url where to find the package repository using URL.")
	else()
		list(LENGTH ADD_PID_PACKAGE_REFERENCE_URL SIZE)
		if(NOT SIZE EQUAL 1)
			message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the git url where to package repository using URL keyword.")
		endif()
		shadow_Repository_Address("${ADD_PID_PACKAGE_REFERENCE_URL}")
	endif()
elseif(ADD_PID_PACKAGE_REFERENCE_BINARY)
	if(NOT ADD_PID_PACKAGE_REFERENCE_URL)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the urls where to find binary packages for release and debug modes, using URL <release addr> <debug addr>.")
	else()
		list(LENGTH ADD_PID_PACKAGE_REFERENCE_URL SIZE)
		if(NOT SIZE EQUAL 2)
			message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the urls where to find binary packages for release and debug modes using URL <release addr> <debug addr>.")
	...
	if(NOT ADD_PID_PACKAGE_REFERENCE_SYSTEM)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the target system name (Linux, MacOS) using SYSTEM keyword.")
	endif()

	if(NOT ADD_PID_PACKAGE_REFERENCE_VERSION)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to input a major and a minor number, optionnaly you can set a patch version (considered as 0 if not set) using VERSION keyword.")
	else()
	...
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to use the function signature SOURCE or BINARY.")
endif()
...
if(NOT ${ARGC} EQUAL 1)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, the add_PID_Package_Category command requires one string argument of the form <category>[/subcategory]*.")
endif()
add_Category("${ARGV0}")
...
if(NOT DECLARE_PID_WIKI_GIT_ADDRESS)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the repository address for the project wiki using GIT_ADDRESS keyword.")
endif()
if(NOT DECLARE_PID_WIKI_HOME_PAGE)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to set the http address of the home page of this wiki using HOME_PAGE keyword.")
endif()
...
if(${ARGC} GREATER 0)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, the build_PID_Package command requires no arguments.")
endif()
build_Package()
...
if(DECLARE_PID_COMPONENT_UNPARSED_ARGUMENTS)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, unknown arguments ${DECLARE_PID_COMPONENT_UNPARSED_ARGUMENTS}.")
endif()

if(NOT DECLARE_PID_COMPONENT_NAME)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a name must be given to the component using NAME keyword.")
endif()
if(NOT DECLARE_PID_COMPONENT_DIRECTORY)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a source directory must be given using DIRECTORY keyword.")
endif()
set(nb_options 0)
...
if(NOT nb_options EQUAL 1)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, only one type among (STATIC_LIB, SHARED_LIB, MODULE_LIB, HEADER_LIB, APPLICATION, EXAMPLE_APPLICATION|TEST_APPLICATION) must be given for the component.")
endif()
...
if(DECLARE_PID_COMPONENT_INTERNAL)
	if(DECLARE_PID_COMPONENT_INTERNAL STREQUAL "")
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, INTERNAL keyword must be followed by by at least one DEFINITION OR INCLUDE_DIR OR LINK keyword and related arguments.")
	endif()
	...
	if(DECLARE_PID_COMPONENT_INTERNAL_LINKS)
		if(type MATCHES HEADER OR type MATCHES STATIC)
			message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, ${type} libraries cannot define internal linker flags.")
		endif()
...
if(DECLARE_PID_COMPONENT_EXPORTED)
	if(type MATCHES APP OR type MATCHES EXAMPLE OR type MATCHES TEST)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, applications cannot export anything (invalid use of the EXPORT keyword).")
	elseif(type MATCHES MODULE)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, module librairies cannot export anything (invalid use of the EXPORT keyword).")
	endif()
	if(DECLARE_PID_COMPONENT_EXPORTED STREQUAL "")
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, EXPORTED keyword must be followed by at least one DEFINITIONS OR LINKS keyword and related arguments.")
	endif()
...
if(NOT DECLARE_PID_DEPENDENCY_PACKAGE)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a name must be given to the required package using PACKAGE keywork.")
endif()

if(DECLARE_PID_DEPENDENCY_EXTERNAL AND DECLARE_PID_DEPENDENCY_NATIVE)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, the type of the required package must be EXTERNAL or NATIVE, not both.")
	...
	if(DECLARE_PID_DEPENDENCY_UNPARSED_ARGUMENTS)
		...
		if(DECLARE_PID_DEPENDENCY_EXTERNAL_EXACT)
			...
			if(NOT DECLARE_PID_DEPENDENCY_EXTERNAL_VERSION)
				message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you must use the EXACT keyword together with the VERSION keyword.")
			endif()
		endif()
		if(DECLARE_PID_DEPENDENCY_EXTERNAL_COMPONENTS)
			list(LENGTH DECLARE_PID_DEPENDENCY_EXTERNAL_COMPONENTS SIZE)
			if(SIZE LESS 1)
				message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, at least one component dependency must be defined when using the COMPONENTS keyword.")
			endif()
		endif()
	endif()
...
elseif(DECLARE_PID_DEPENDENCY_NATIVE)
	if(DECLARE_PID_DEPENDENCY_UNPARSED_ARGUMENTS)
		...
		if(DECLARE_PID_DEPENDENCY_PID_UNPARSED_ARGUMENTS)
			message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, there are some unknown arguments ${DECLARE_PID_DEPENDENCY_NATIVE_UNPARSED_ARGUMENTS}.")
		endif()
		...
		if(DECLARE_PID_DEPENDENCY_NATIVE_VERSION)
			...
			elseif(SIZE EQUAL 1)#it is a complete version string
				...
				message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you need to input a major and a minor number.")
			endif()
		...
		if(DECLARE_PID_DEPENDENCY_NATIVE_COMPONENTS)
			...
			if(SIZE LESS 1)
				message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, at least one component dependency must be defined when using COMPONENTS keyword.")
			endif()
		endif()
...
if(DECLARE_PID_COMPONENT_DEPENDENCY_UNPARSED_ARGUMENTS)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, unknown arguments ${DECLARE_PID_COMPONENT_DEPENDENCY_UNPARSED_ARGUMENTS}.")
endif()
if(NOT DECLARE_PID_COMPONENT_DEPENDENCY_COMPONENT)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a name must be given to the component that declare the dependency using COMPONENT keyword.")
endif()
set(export FALSE)
...
if(DECLARE_PID_COMPONENT_DEPENDENCY_LINKS)
	...
	if(DECLARE_PID_COMPONENT_DEPENDENCY_LINKS_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, the LINKS option argument must be followed only by static and/or shared links.")
	endif()
...
if(DECLARE_PID_COMPONENT_DEPENDENCY_DEPEND OR DECLARE_PID_COMPONENT_DEPENDENCY_NATIVE)
	if(DECLARE_PID_COMPONENT_DEPENDENCY_EXTERNAL)
		message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, keywords EXTERNAL (requiring an external package) and NATIVE (or DEPEND) (requiring a PID component) cannot be used simultaneously.")
	endif()
...
if(NOT RUN_PID_TEST_NAME)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a name must be given to the test (using NAME <name> syntax) !")
endif()

if(NOT RUN_PID_TEST_EXE AND NOT RUN_PID_TEST_COMPONENT)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, an executable must be defined. Using EXE you can use an executable present on your system or by using COMPONENT. In this later case you must specify a PID executable component. If the PACKAGE keyword is used then this component will be found in another package than the current one.")
endif()

if(RUN_PID_TEST_EXE AND RUN_PID_TEST_COMPONENT)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, you must use either a system executable (using EXE keyword) OR a PID application component (using COMPONENT keyword).")
endif()
...
if(NOT EXT_PACKAGE_PATH_NAME OR NOT EXT_PACKAGE_PATH_PATH)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a name of an external package must be provided with name and a variable containing the resulting path must be set with PATH keyword.")
endif()
...
if(NOT CREATE_INSTALL_SYMLINK_NAME OR NOT CREATE_INSTALL_SYMLINK_PATH OR NOT CREATE_INSTALL_SYMLINK_TARGET)
	message(FATAL_ERROR "[PID notification] CRITICAL ERROR : bad arguments, a name for the new symlink created must be provided with NAME keyword, the path relative to its install location must be provided with PATH keyword and the target of the symlink must be provided with TARGET keyword.")
endif()
