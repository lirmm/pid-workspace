...
set(oneValueArgs VERSION OS ARCH)
...
if(NOT ADD_PID_PACKAGE_REFERENCE_OS)
	message(FATAL_ERROR "[PID] CRITICAL ERROR : bad arguments, you need to set the target operating system name (linux or macosx) using OS keyword.")
endif()

if(NOT ADD_PID_PACKAGE_REFERENCE_ARCH)
...
if(NOT ADD_PID_PACKAGE_REFERENCE_VERSION)
	...
	if(NOT PATCH)
		add_Reference("${MAJOR}.${MINOR}.0" "${ADD_PID_PACKAGE_REFERENCE_OS}" "${ADD_PID_PACKAGE_REFERENCE_ARCH}" "${URL_REL}" "${URL_DBG}")
	else()
		add_Reference("${MAJOR}.${MINOR}.${PATCH}" "${ADD_PID_PACKAGE_REFERENCE_OS}" "${ADD_PID_PACKAGE_REFERENCE_ARCH}" "${URL_REL}" "${URL_DBG}")
	endif()
endif()
