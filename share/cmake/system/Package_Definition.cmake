...
if(ADD_PID_PACKAGE_REFERENCE_SOURCE)
	if(NOT ADD_PID_PACKAGE_REFERENCE_URL)
		message(FATAL_ERROR "[PID] CRITICAL ERROR : bad arguments, you need to set the url where to find the package repository using URL.")
	else()
		list(LENGTH ADD_PID_PACKAGE_REFERENCE_URL SIZE)
		if(NOT SIZE EQUAL 1)
			message(FATAL_ERROR "[PID] CRITICAL ERROR : bad arguments, you need to set the git url where to package repository using URL keyword.")
		endif()
...
elseif(ADD_PID_PACKAGE_REFERENCE_BINARY)
	...
	if(NOT ADD_PID_PACKAGE_REFERENCE_SYSTEM)
		message(FATAL_ERROR "[PID] CRITICAL ERROR : bad arguments, you need to set the target system name (Linux, MacOS) using SYSTEM keyword.")
	endif()

	if(NOT ADD_PID_PACKAGE_REFERENCE_VERSION)
		message(FATAL_ERROR "[PID] CRITICAL ERROR : bad arguments, you need to input a major and a minor number, optionnaly you can set a patch version (considered as 0 if not set) using VERSION keyword.")
	else()
	...
	message(FATAL_ERROR "[PID] CRITICAL ERROR : bad arguments, you need to use the function signature SOURCE or BINARY.")
endif()
