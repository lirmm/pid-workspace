...
if(TO_INSTALL_EXTERNAL_DEPS) #there are dependencies to install
	if(REQUIRED_PACKAGES_AUTOMATIC_DOWNLOAD)
		...
		message(FATAL_ERROR "[PID notification] :  there are some unresolved required external package dependencies : ${${PROJECT_NAME}_TOINSTALL_EXTERNAL_PACKAGES${VAR_SUFFIX}}. You may download them \"by hand\" or use the required packages automatic download option")
		return()
	endif()
endif()
	...
	if(REQUIRED_PACKAGES_AUTOMATIC_DOWNLOAD)
		...
		foreach(installed IN ITEMS ${INSTALLED_PACKAGES})#recursive call for newly installed packages
			...
			if(${installed}_FOUND)
				...
				message(FATAL_ERROR "[PID notification] :  impossible to find installed package ${installed}")
			endif()	
		endforeach()
	else()	
		message(FATAL_ERROR "[PID notification] :  there are some unresolved required package dependencies : ${${PROJECT_NAME}_TOINSTALL_PACKAGES${VAR_SUFFIX}}. You may download them \"by hand\" or use the required packages automatic download option")
		return()
	endif()
endif()
...
if(not_installed)
	message("[PID notification] :  ERROR, Some of the required packages cannot be installed : ${not_installed}")
endif()
...
if(IS_EXISTING)
	...
	if(IS_EXISTING)
		...
		message("[PID notification] :  ERROR, Unknown package ${package} : cannot find any source or reference of this package in the workspace")
		return()
	endif()
endif()
...
if(USE_SOURCES) #package sources reside in the workspace
	...
	if(NOT ${refer_path} STREQUAL NOTFOUND)
		...
		message("[PID notification] :  ERROR, reference file not found for package ${package}!! This is maybe due to a bad release of package ${package}. Please contact the administrator of this package. !!!")
	endif()
	...
	if(PACKAGE_BINARY_DEPLOYED) # if there is ONE adequate reference, downloading and installing it
		...
		if(DEPLOYED) # doing the same as for the USE_SOURCES step
			...
			if(NOT SOURCE_DEPLOYED)
				set(${INSTALL_OK} FALSE PARENT_SCOPE)				
				message("[PID notification] :  ERROR, impossible to build the package sources ${package}. Try \"by hand\".")
				return()
			endif()
			set(${INSTALL_OK} TRUE PARENT_SCOPE)
		else()
...
if(${package}_ADDRESS)
	...
	message("[PID notification] :  ERROR, impossible to clone the repository of package ${package} (no repository address defined). This is maybe due to a malformed package, please contact the administrator of this package.")
endif()
...
if(NOT numeric_error EQUAL 0)
	...
	message("[PID notification] :  ERROR, problem when downloading binary version ${version_string} of package ${package} from address ${download_url}: ${status}")
	return()
endif()
...
if(NOT numeric_error_dbg EQUAL 0)#there is an error
	set(${INSTALLED} FALSE PARENT_SCOPE)
	message("[PID notification] :  ERROR, problem when downloading binary version ${version_string} of package ${package} from address ${download_url_dbg} : ${status_dbg}")
	return()
endif()
...
if(NOT RES_VERSION)
	message("[PID notification] :  ERROR, no version found for package ${package} !! Maybe this is due to a malformed package. Please contact the administrator of this package.")
	restore_Repository_Context(${package} ${CURRENT_COMMIT} ${SAVED_CONTENT})
	return()
endif()
...
if(ALL_IS_OK)
	...
	message("[PID notification] :  ERROR, automatic build and install of package ${package} FAILED !!")
endif()
restore_Repository_Context(${package} ${CURRENT_COMMIT} ${SAVED_CONTENT})
...
if(NOT ${PROJECT_NAME}_TOINSTALL_EXTERNAL_${package}_VERSIONS${USE_MODE_SUFFIX})#no specific version required
	if(${package}_REFERENCES)
		...
		message("[PID notification] :  ERROR, Impossible to find a valid reference to any version of external package ${package}")
		return()
	endif()
...
if(NOT IS_EXISTING)
	set(${INSTALL_OK} FALSE PARENT_SCOPE)
	message("[PID notification] :  ERROR, Unknown external package ${package} : cannot find any reference of this package in the workspace")
	return()
endif()
include(ReferExternal${package} OPTIONAL RESULT_VARIABLE refer_path)
if(${refer_path} STREQUAL NOTFOUND)
	message("[PID notification] :  ERROR, Reference file not found for package ${package}!! This is certainly due to a bad released package. Please contact the administrator or that package !!!")
	return()
endif()
...
if(SELECTED)
	...
	message("[PID notification] :  ERROR, impossible to find an adequate version for external package ${package}")
endif()
...
if(not_installed)
	message("[PID notification] :  ERROR, Some of the required external packages cannot be installed : ${not_installed}")
endif()
...
get_System_Variables(OS_STRING PACKAGE_STRING)
###### downloading the binary package ######
message("[PID notification] : downloading the binary package, please wait ...")
#1) release code
set(FILE_BINARY "")
set(FOLDER_BINARY "")
...
if(NOT numeric_error EQUAL 0)
	set(${DEPLOYED} FALSE PARENT_SCOPE)
	message("[PID notification] :  ERROR, problem when downloading binary version ${VERSION} of package ${package} from address ${download_url}: ${status}")
	return()
endif()
...
if(EXISTS ${package}_REFERENCE_${VERSION}_${OS_STRING}_url_DEBUG)
	...
	if(NOT numeric_error_dbg EQUAL 0)#there is an error
		set(${DEPLOYED} FALSE PARENT_SCOPE)
		message("[PID notification] :  ERROR, problem when downloading binary version ${VERSION} of package ${package} from address ${download_url_dbg} : ${status_dbg}")
		return()
	endif()
endif()
...
set(error_res "")
message("[PID notification] : decompressing the binary package, please wait ...")
...
if (error_res)
	set(${DEPLOYED} FALSE PARENT_SCOPE)
	message("[PID notification] :  ERROR,  cannot extract binary archives ${FILE_BINARY} ${FILE_BINARY_DEBUG}")
	return()
endif()
...
if (error_res)
	set(${INSTALLED} FALSE PARENT_SCOPE)
	message("[PID notification] :  ERROR, cannot extract folder from ${FOLDER_BINARY} ${FOLDER_BINARY_DEBUG}")
	return()
endif()
